...
from model_elements.intersection_hot_mijnversie import Intersection
...
class FugitiveModel(DSOLModel):
    def __init__(self, simulator, input_params, seed=None):
        ...
        self.graph_onvoorzien = input_params['graph_onvoorzien']
        ...
        self.traffic_lights = input_params['traffic_lights']
        self.traffic_light_categories = input_params['traffic_light_categories']
        self.traffic_light_configuration = input_params['traffic_light_configuration']
        self.cross_green_wave_road = input_params['cross_green_wave_road']
        self.green_wave_road = input_params['green_wave_road']
        self.nodes_onvoorzieneomstandigheid = input_params['nodes_onvoorzieneomstandigheid']
        self.roundabouts = input_params['roundabouts']
    ...
    def construct_nodes(self):
        for node, data in self.graph.nodes(data=True):
            ...
            component = Intersection(simulator=self.simulator,
                                     ...
                                     name=node,
                                     graph=self.graph,
                                     graph_onvoorzien=self.graph_onvoorzien,
                                     jitter=self.jitter,
                                     escape_nodes=self.escape_nodes,
                                     traffic_lights=self.traffic_lights,
                                     traffic_light_categories=self.traffic_light_categories,
                                     traffic_light_configuration=self.traffic_light_configuration,
                                     cross_green_wave_road=self.cross_green_wave_road,
                                     green_wave_road=self.green_wave_road,
                                     nodes_onvoorzieneomstandigheid=self.nodes_onvoorzieneomstandigheid,
    ...
    def construct_links(self):
        for i, (u, v, data) in enumerate(self.graph.edges(data=True)):
            ...
            road = Road(simulator=self.simulator,
                        ...
                        destination_name=v,
                        length=data['travel_time_with_congestion'],
                        selection_weight=1,
                        graph=self.graph,
            ...
            # TODO ignoring one-way #als ze verkeerd zijn gereden kunnen ze de geldende richting negeren
